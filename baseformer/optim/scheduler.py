...
def get_lr_linear_schedule(
    ...
    The schedule has three phases:
    - Warm-up (t < T_w): α_t = (t / T_w) * α_max
    - Linear decay (T_w <= t <= T_l): α_t = α_max + (α_min - α_max) * (t - T_w) / (T_l - T_w)
    - Post-decay (t > T_l): α_t = α_min
    ...
    Args:
        ...
        min_learning_rate: Minimum (final) learning rate α_min.
        warmup_iters: Number of warm-up iterations T_w.
        linear_cycle_iters: Iteration number at which linear decay ends T_l.
    ...
    if it < warmup_iters:
        return max_learning_rate * it / warmup_iters
    elif it <= linear_cycle_iters:
        progress = (it - warmup_iters) / (linear_cycle_iters - warmup_iters)
        return max_learning_rate + (min_learning_rate - max_learning_rate) * progress
    else:
        return min_learning_rate
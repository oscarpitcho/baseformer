...
class Trainer:
    ...
    def train(self) -> None:
        ...
        while self.step < self.cfg.experiment.max_steps:
            ...
            if self.step % 10 == 0:
                ...
                log.info(f"Step {self.step}: loss={loss:.4f}, lr={curr_lr:.2e}, tokens={self.tokens_processed:,}")


            # Evaluation
            if (self.val_loader is not None) and (self.step % self.cfg.experiment.eval_every == 0) and (self.step > 0):
    ...
    def _train_step(self, batch: dict) -> tuple[float, int, float]:
        ...
        Returns:
            Tuple of (loss value, number of tokens in batch, learning rate).
        """
        ...
        loss = loss_cross_ent(logits, labels)
        ...
        lr = self.optimizer.step(self.lr_schedule)

        return loss.item(), labels.numel()
    ...
    def _eval(self) -> dict:
        ...
        batches = [next(val_iter) for _ in range(100)]
        sampled_batches = random.sample(batches, 10)
        for sampled_batch in sampled_batches:
            input_ids = sampled_batch["input_ids"].to(self.device)
            labels = sampled_batch["labels"].to(self.device)

            logits = self.model(input_ids)
            total_loss += loss_cross_ent(logits, labels).item()
            total_ppl += perplexity(logits, labels).item()
            num_batches += 1
